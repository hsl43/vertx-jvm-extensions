...
public class EventBusExtensionsTestJ extends VertxPlatform {
  ...
  @Test
  public void sendJsonObject_send_address_is_message_class_fqn(@NotNull TestContext context) {
    final Async async = context.async();
    ...
    final Handler<AsyncResult<Message<JsonObject>>> handler = reply -> {
      if(reply.succeeded()) {
        consumer.unregister();
    ...
    EventBusExt.sendJsonObject(eventBus, message, options, handler);
  }

  @Test
  public void sendJsonObject_sends_message_as_json_object(@NotNull TestContext context) {
    final Async async = context.async();
    ...
    final Handler<AsyncResult<Message<JsonObject>>> handler = reply -> {
      if(reply.succeeded()) {
        final Pong pong = MessageExt.fromJsonObject(reply.result(), Pong.class);

        context.assertEquals(new Pong().value, pong.value);
